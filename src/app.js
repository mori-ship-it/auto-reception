...
document.addEventListener('DOMContentLoaded',function(){
  firebase.initializeApp({apiKey:"AIzaSyDopvxzyHEcm8KChamvVVaN9YVxHCamGx0",authDomain:"salon-reception.firebaseapp.com",projectId:"salon-reception",storageBucket:"salon-reception.firebasestorage.app",messagingSenderId:"931290276865",appId:"1:931290276865:web:a855857799d47926d70c00"});
  db=firebase.firestore();loadFromStorage();
});
...
function applyLang(){Object.keys(TX.ja).forEach(function(k){var el=document.getElementById('t-'+k);if(el)el.innerHTML=tx(k);});document.getElementById('nameInput').placeholder=tx('name-ph');document.getElementById('errMsg').textContent=tx('err');document.getElementById('stylistSearch').placeholder=tx('search-ph');document.getElementById('langBtn').value=lang;applyCustom();onStylistSearch(document.getElementById('stylistSearch').value);}
...
function openHomePanel(){
  ...
  adminDirty=false;populatePreview();
...
function onDrop(e,tid){e.preventDefault();if(dragSrcId===tid)return;var si=staffList.findIndex(function(s){return s.id===dragSrcId;});var ti=staffList.findIndex(function(s){return s.id===tid;});var nl=staffList.slice();var rm=nl.splice(si,1)[0];nl.splice(ti,0,rm);staffList=nl;renderAdminStaff();}
...
function addLog(name,type,stylist){visitLog.unshift({time:nowFull(),name:name,type:type,stylist:stylist||null});saveToStorage();}
...
async function saveToStorage(){
  try{
    ...
    await db.collection('salon').doc(STORE_ID).set({custom:custom,pinCode:pinCode,webhookUrl:webhookUrl,botToken:botToken,staffList:staffList,nextStaffId:nextStaffId,txCache:{en:TX.en,zh:TX.zh,ko:TX.ko,es:TX.es}},{merge:true});
...
async function loadFromStorage(){
  try{
    ...
    if(snap.exists){var d=snap.data();if(d.custom)Object.assign(custom,d.custom);if(d.pinCode)pinCode=d.pinCode;if(d.webhookUrl!==undefined)webhookUrl=d.webhookUrl;if(d.botToken!==undefined)botToken=d.botToken;if(d.staffList&&d.staffList.length>0)staffList=d.staffList;if(d.nextStaffId)nextStaffId=d.nextStaffId;if(d.txCache){if(d.txCache.en)Object.assign(TX.en,d.txCache.en);if(d.txCache.zh)Object.assign(TX.zh,d.txCache.zh);if(d.txCache.ko)Object.assign(TX.ko,d.txCache.ko);if(d.txCache.es)Object.assign(TX.es,d.txCache.es);}}
    ...
    firestoreReady=true;applyLang();
  }catch(e){console.warn('Storage load error:',e);firestoreReady=true;applyLang();}
}

// ===== ANALYTICS =====
var analyticsData=[],analyticsRange='day',analyticsStylistFilter='all';
...
async function fetchLogsForRange(days){var entries=[];var n=new Date();for(var i=0;i<days;i++){var d=new Date(n);d.setDate(d.getDate()-i);var key=d.getFullYear()+'-'+(d.getMonth()+1)+'-'+d.getDate();var docId=STORE_ID+'_'+key;var dow=d.getDay();if(i===0){visitLog.forEach(function(e){var copy=Object.assign({},e);copy._dow=dow;copy._date=key;entries.push(copy);});}else{try{var snap=await db.collection('logs').doc(docId).get();if(snap.exists&&snap.data().entries){snap.data().entries.forEach(function(e){var copy=Object.assign({},e);copy._dow=dow;copy._date=key;entries.push(copy);});}}catch(e){}}}return entries;}
...
async function initAnalytics(){try{await loadChartJs();}catch(e){return;}setAnalyticsRange('day');}
async function setAnalyticsRange(range){analyticsRange=range;['day','week','month'].forEach(function(r){var btn=document.getElementById('ar-'+r);if(btn){if(r===range)btn.classList.add('active');else btn.classList.remove('active');}});var loading=document.getElementById('analyticsLoading');if(loading)loading.style.display='inline';analyticsData=await fetchLogsForRange(range==='day'?1:(range==='week'?7:30));analyticsStylistFilter='all';if(loading)loading.style.display='none';renderStylistFilter();renderAnalytics();var dowSec=document.getElementById('dowSection');if(dowSec)dowSec.style.display=range==='day'?'none':'block';}
function renderStylistFilter(){var names=['all'];analyticsData.forEach(function(e){var sn=e.stylist||'指名なし';if(names.indexOf(sn)===-1)names.push(sn);});var el=document.getElementById('stylistFilter');el.innerHTML=names.map(function(n){var label=n==='all'?'全員':n;var cls=n===analyticsStylistFilter?'edit-tab active':'edit-tab';return '<button class="'+cls+'" onclick="filterAnalyticsStylist(\''+n.replace(/'/g,"\\'")+'\')">'+label+'</button>';}).join('');}
function filterAnalyticsStylist(name){analyticsStylistFilter=name;renderStylistFilter();renderAnalytics();}
function renderAnalytics(){
  if(!window.Chart)return;var data=getFiltered();var total=data.length;
  ...
  var hourCounts=new Array(24).fill(0);data.forEach(function(e){var h=getHour(e.time);if(h>=0)hourCounts[h]++;});
  var peakH=0,peakV=0;hourCounts.forEach(function(v,i){if(v>peakV){peakV=v;peakH=i;}});
...
function showAnalyticsDrill(hourIdx){
  ...
  var tc={reserved:'#1D9E75',walkin:'#BA7517',call:'#E24B4A',vendor:'#378ADD'};
  var tl={reserved:'予約',walkin:'飛込み',call:'呼出し',vendor:'業者'};
  document.getElementById('drillList').innerHTML=data.map(function(e){var dn=e.name||(e.type==='walkin'?'飛び込み':e.type==='vendor'?'業者':'—');return '<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--border);font-size:12px;"><span style="flex:1;">'+dn+'</span><span style="color:'+(tc[e.type]||'var(--text-muted)')+';font-size:10px;background:var(--accent-light);padding:2px 8px;border-radius:6px;">'+(tl[e.type]||e.type)+'</span><span style="color:var(--text-muted);font-size:10px;">'+(e.stylist||'—')+'</span></div>';}).join('');
  document.getElementById('drillPanel').style.display='block';
}
function renderVisitList(data){
  var el=document.getElementById('analyticsVisitList');if(!el)return;
  if(!data.length){el.innerHTML='<div class="log-empty">データがありません</div>';return;}
  var tc={reserved:'#1D9E75',walkin:'#BA7517',call:'#E24B4A',vendor:'#378ADD'};
  var tl={reserved:'予約',walkin:'飛込み',call:'呼出し',vendor:'業者'};
  el.innerHTML=data.slice(0,50).map(function(e){var dn=e.name||(e.type==='walkin'?'飛び込み':e.type==='vendor'?'業者':'—');return '<div class="log-card"><span class="log-time" style="width:60px;">'+e.time+'</span><span class="log-name" style="font-size:13px;">'+dn+'</span><span style="font-size:10px;color:var(--text-muted);">'+(e.stylist||'')+'</span><span class="log-badge '+e.type+'" style="font-size:9px;">'+(tl[e.type]||'')+'</span></div>';}).join('')+(data.length>50?'<div style="text-align:center;font-size:10px;color:var(--text-muted);padding:12px;">他 '+(data.length-50)+'件</div>':'');
}
